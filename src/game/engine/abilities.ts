...
import { type Ctx, getHero, getSpellZone, livingHeroes, makeInstanceId, opposite, playerOf, push, setHero, setSpellZone } from './board';
...
/** True if the Hero at (side, lane) currently has a live PASSIVE ability granting immunity to `kind`, targeting itself. A silenced Hero's PASSIVE abilities never apply. */
  ...
  for (const ability of card.abilities) {
    ...
    for (const action of ability.actions) {
      ...
      for (const loc of resolveTargetLocations(ctx, exec, action.target)) {
        if (loc.side === side && loc.lane === lane) return true;
      }
    }
  }
  return false;
...
function filterHostileTargets(ctx: Ctx, exec: AbilityContext, action: ActionDef, locs: { side: Side; lane: LaneId }[]): { side: Side; lane: LaneId }[] {
  ...
  return locs.filter((loc) => {
    ...
    if (hasImmunity(ctx, loc.side, loc.lane, immunity)) {
...
function evalOneCondition(ctx: Ctx, exec: AbilityContext, condition: ConditionDef): boolean {
  switch (condition.type) {
    ...
    case 'ALLY_DIED_THIS_ROUND':
      return ctx.events.some((e) => e.type === 'HERO_DESTROYED' && e.side === exec.ownerSide);
    case 'ENEMY_DIED_THIS_ROUND':
      return ctx.events.some((e) => e.type === 'HERO_DESTROYED' && e.side === opposite(exec.ownerSide));
...
function computeCountBasis(ctx: Ctx, side: Side, basis: CountBasis, faction: Faction | undefined): number {
  switch (basis) {
...
function executeAction(ctx: Ctx, action: ActionDef, exec: AbilityContext): void {
  switch (action.type) {
    ...
    case 'CHANGE_POWER_BY_COUNT': {
      const count = computeCountBasis(ctx, exec.ownerSide, action.basis, action.faction);
    ...
    case 'DEBUFF_ALL_OTHERS': {
      ...
      for (const side of ['player', 'enemy'] as Side[]) {
        for (const { lane, hero } of livingHeroes(ctx, side)) {
          ...
          if (hostile && hasImmunity(ctx, side, lane, immunity)) {
...
function pickEligibleFromGraveyard(graveyard: string[], maxPower: number, pick: GraveyardPick, ctx: Ctx, faction?: Faction): { cardId: string; index: number } | null {
  const eligible = graveyard
    .map((cardId, index) => ({ cardId, index, power: getCard(cardId).power ?? Infinity, faction: getCard(cardId).faction }))
    .filter((e) => e.power <= maxPower && (!faction || e.faction === faction));
...
function removeAndRecord(ctx: Ctx, side: Side, lane: LaneId): DeadHero | null {
  ...
  return { side, lane, instanceId: hero.instanceId, cardId: hero.cardId, name: hero.name, silenced: hero.silenced };
}
...
export function destroyAndChain(ctx: Ctx, entries: { side: Side; lane: LaneId }[]): void {
  ...
  while (queue.length > 0) {
    ...
    for (const ally of livingHeroes(ctx, dead.side)) dispatchTriggerForHero(ctx, dead.side, ally.lane, 'ON_ALLY_DEATH');
    for (const foe of livingHeroes(ctx, opposite(dead.side))) dispatchTriggerForHero(ctx, opposite(dead.side), foe.lane, 'ON_ENEMY_DEATH');
    for (const lane of ['left', 'center', 'right'] as LaneId[]) {
 ...
 */
export function dealOverflowDamage(ctx: Ctx, winnerSide: Side, lane: LaneId, amount: number, winnerName: string, loserName: string): void {
  if (amount <= 0) return;
  const loserSide = opposite(winnerSide);
  const p = playerOf(ctx, loserSide);
  const from = p.hp;
  p.hp = Math.max(0, p.hp - amount);
  push(ctx, { type: 'OVERFLOW_DAMAGE', side: loserSide, lane, amount, from, to: p.hp, winnerName, loserName });
}
